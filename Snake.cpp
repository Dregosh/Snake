...
void Snake::Extend()
{
	...
	//referencja na ostatni element wektora:
	SnakeSegment& tail_head = m_snakeBody[m_snakeBody.size() - 1];
	if (m_snakeBody.size() > 1)
	{
		//referencja na przedostatni element wektora:
		SnakeSegment& tail_bone = m_snakeBody[m_snakeBody.size() - 2];
		//(jezeli rozmiar weza jest >1 to potrzebna jest nam tez referencja do elementu przedostatniego aby znac kierunek calego ogona)
...
void Snake::Reset()
{
	m_snakeBody.clear();	//czysci wektor segmentow 
...
void Snake::Cut(int l_segments)
{
	...
	--m_lives;
	m_log->Add("Tracisz 1 zycie! Masz jeszcze: "
